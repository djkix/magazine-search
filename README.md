...
- **Sommaire thématique par collection** (optionnel, admin) : en plus du sommaire classique par numéro, génère à la demande via Gemini un regroupement par thématique de tous les articles de la collection (ex. « Automobile », « Santé »), pour voir d'un coup d'œil tout ce qui a été traité sur un sujet à travers les numéros.
...
- **Parcourir** : `/library` et `/articles` (sommaires) présentent d'abord les collections, puis le détail des numéros ou des sommaires de la collection choisie, triable par date ou par type (normal/HS/spécial). Sur `/articles`, un admin peut générer un sommaire thématique (bouton « Générer »/« Régénérer », onglet « Par thématique ») qui regroupe tous les articles de la collection par sujet via Gemini.
