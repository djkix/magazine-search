...
- **Viewer PDF intégré** (`pdf.js`) en défilement continu, saut direct à une page ou à un article du sommaire, overlay de surlignage des termes recherchés. En arrivant depuis un résultat de recherche, la colonne de gauche liste les autres magazines correspondant à la même recherche (avec leur nombre d'occurrences et leur date de publication, pour repérer en un coup d'œil si un résultat très cité est récent ou ancien) pour naviguer entre eux sans revenir à la page de recherche. Le PDF est chargé par requêtes HTTP Range (le serveur les honore nativement) : `pdf.js` ne télécharge que les octets nécessaires à la page affichée plutôt que tout le fichier (30-75 Mo) d'un coup, ce qui accélère surtout l'ouverture sur mobile.
